...
(function () {
  ...
  async function init() {
    try {
      ...
      if (regResponse && regResponse.success) {
        ...
        console.warn('BOTC Grimoire Hub: Could not load remote registry. Falling back to empty.');
        registry = { themes: [] };
      }

      // Check if we are running on the website portal instead of the game client
      ...
      if (isWebsite) {
        ...
        console.log('BOTC Grimoire Hub: Userscript website integration active.');
      ...
      console.log('BOTC Grimoire Hub (Userscript) initialized successfully!');
    } catch (e) {
      console.error('BOTC Grimoire Hub (Userscript): Initialization failed:', e);
    }
  }

  // --- Userscript Storage Settings Helpers ---
  function getSettings() {
    return Promise.resolve({
      autoDetect: GM_getValue('autoDetect', true),
      activeThemeId: GM_getValue('activeThemeId', null)
    });
  }
        ...
        GM_xmlhttpRequest({
          ...
          onload: function(response) {
            if (response.status === 200) {
              ...
              resolve({ success: false, error: `HTTP status ${response.status}` });
            }
          },
  ...
  function createFloatingUI() {
    ...
    floatBtn.innerHTML = `
      <img src="https://raw.githubusercontent.com/cijamie/grimoirepick/main/logo.png" alt="Logo" style="width: 28px; height: 28px; object-fit: contain; pointer-events: none;">
    `;
  ...
  function renderThemes(searchQuery = '') {
    ...
    if (filteredThemes.length === 0) {
      container.innerHTML = `<div style="text-align: center; color: #90a4ae; padding: 20px; font-size: 0.8rem;">No themes match query.</div>`;
      return;
    }
      ...
      if (cat.items.length === 0) {
        ...
        cat.items.forEach(theme => {
          ...
          card.innerHTML = `
            ...
            ${theme.scriptName ? `<div class="theme-script">${theme.scriptName}</div>` : ''}
